...
import "./CarouselExtra.css";

// ─── Logos para o Carrosel ─────────────────────────────────────────────────
...
export default function CarouselExtra({
  items = DEFAULT_ITEMS,
  baseWidth = 300,
  autoplay = false,
  autoplayDelay = 3000,
  ...
  const maxIndex = Math.max(0, itemsForRender.length - 1);
  
  let newPosition = position;

  if (position > maxIndex || (loop && position === 0)) {
...
}, [items.length, loop, trackItemOffset, itemsForRender.length, x]);
